...
$myQry = "SELECT address, inventory, damages, calendar, calendarNotes, managerID, cFee, cPercent FROM listings WHERE id = ?";
...
if ($stmt = mysqli_prepare($link, $myQry)) {
  mysqli_stmt_bind_param($stmt, "s", $param_id);
    ...
    mysqli_stmt_bind_result($stmt, $address, $inventory, $damages, $calendar, $calendarNotes, $managerID, $cFee, $cPercent);
                                        ...
                                        <div class="row transFormRow">
                                            ...
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    ...
                                                    <input type="number" name="total" step=".01" class="form-control" id="transTotal" placeholder="Enter total amount">
                                                </div>
                                            </div>
                                        </div>
                                        <div class="row transFormRow">
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    ...
                                                    <input type="number" name="cleaning" step=".01" class="form-control" id="transCleaning" placeholder="Enter cleaning fee" value="<?php echo $cFee?>">
                                                </div>
                                            </div>
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    <label for="transEarnings">Earnings</label>
                                                    <input type="number" name="earnings" step=".01" class="form-control" id="transEarnings" placeholder="Enter earnings">
                                                </div>
                                            </div>
                                        </div>
                                        <div class="row transFormRow">
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    ...
                                                    <input type="number" name="commission" step=".01" class="form-control" id="transCommission" placeholder="Enter manager commission">
                                                </div>
                                            </div>
                                            <div class="col-md-6">
                                                <div class="form-group input-group input-daterange " style="padding: 25px 0px;">
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <div class="card card-user">
                                <div class="image">
                                    <img src="/assets/img/manager_bg.png"
                                        alt="...">
                                </div>
                                <div class="content">
                                    <div class="author">
...
<script type="text/javascript ">
    ...
    function fillCalendar(){
        ...
        for(var i in calendarArray){
            var inD = new Date(calendarArray[i][0]);
            inD.setDate(inD.getDate()-1);
            var out = new Date(calendarArray[i][1]);
            $(".fc-day-top").each(function(){
    ...
    function sortDates(){
        var items = Object.keys(dates).map(function(key) {
            return [dates[key][1], dates[key][2]];
        });
        items.sort()
        calendarArray = items;
    ...
    function editTrans(id){
        ...
        status.removeClass("Due");
        status.removeClass("InvoicePending");
        status.css("color","black");
        if(status.html() == "Paid"){
            status.html("<select><option value='Upcoming'>Upcoming</option><option value='Due'>Due</option><option value='Late'>Late</option><option value='Paid' selected>Paid</option><option value='InvoicePending'>InvoicePending</option></select>");
        }else if(status.html() == "Late"){
            status.html("<select><option value='Upcoming'>Upcoming</option><option value='Due'>Due</option><option value='Late' selected>Late</option><option value='Paid'>Paid</option><option value='InvoicePending'>InvoicePending</option></select>");
        }else if(status.html() == "Due"){
            status.html("<select><option value='Upcoming'>Upcoming</option><option value='Due' selected>Due</option><option value='Late'>Late</option><option value='Paid'>Paid</option><option value='InvoicePending'>InvoicePending</option></select>");
        }else if(status.html() == "Upcoming"){
            status.html("<select><option value='Upcoming' selected>Upcoming</option><option value='Due'>Due</option><option value='Late'>Late</option><option value='Paid'>Paid</option><option value='InvoicePending'>InvoicePending</option></select>");
        }else{
            status.html("<select><option value='Upcoming'>Upcoming</option><option value='Due'>Due</option><option value='Late'>Late</option><option value='Paid'>Paid</option><option value='InvoicePending' selected>InvoicePending</option></select>");
        }
    ...
    $(function () {
        ...
        $("#transTotal").change(function(){
            if("<?php echo $cPercent?>" != "0" && "<?php echo $cFee?>" != "0.00" && $("#transTotal").val() != ""){
                var c = <?php echo $cPercent?>;
                var t = $("#transTotal").val() - $("#transCleaning").val();
                $("#transEarnings").val(decimal(((100-c - (c*.05))/100) * t));
                $("#transCommission").val(decimal(((c*1.05)/100) * t));
            }
        });
        $("#transCleaning").change(function(){
            if("<?php echo $cPercent?>" != "0" && "<?php echo $cFee?>" != "0.00" && $("#transTotal").val() != ""){
                var c = <?php echo $cPercent?>;
                var t = $("#transTotal").val() - $("#transCleaning").val();
                $("#transEarnings").val(decimal(((100-c - (c*.05))/100) * t));
